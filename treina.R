...
agregado_tudo_novo_1$População <- str_replace(agregado_tudo_novo_1$População, '\\.', '')

agregado_tudo_novo_1$População <- sapply(agregado_tudo_novo_1$População, as.numeric)
...
agregado_tudo_novo_1$Total_de_crimes <- as.numeric(agregado_tudo_novo_1$Total_de_crimes)
agregado_tudo_novo_1$População <- as.numeric(agregado_tudo_novo_1$População)

agregado_tudo_novo_1$crime_per_capita <- (agregado_tudo_novo_1$Total_de_crimes / agregado_tudo_novo_1$População) * 100000 

agregado_tudo_novo_1$crime_per_capita

df_1 <- agregado_tudo_novo_1[c("name", "Total_de_crimes", "tempo", "População", "crime_per_capita")]
 

write.csv(df_1, "agregado_tudo_novo_6.csv", row.names = FALSE)
...
matriz = cbind(crime_per_capita, Total_de_crimes1m, Total_de_crimes2m, População,'60 e mais', matriculas_creche_municipal, matriculas_ESAI_estadual, matriculas_ESAF_municipal, matriculas_EM_municipal)
str(matriz)
matriz <- data.frame(crime_per_capita, Total_de_crimes1m, Total_de_crimes2m, População, `60 e mais`, matriculas_creche_municipal, matriculas_ESAI_estadual, matriculas_ESAF_municipal, matriculas_EM_municipal)

matriz$crime_per_capita <- as.numeric(matriz$crime_per_capita)
matriz$Total_de_crimes1m <- as.numeric(matriz$Total_de_crimes1m)
matriz$Total_de_crimes2m <- as.numeric(matriz$Total_de_crimes2m)
matriz$População <- as.numeric(matriz$População)
unique(matriz$"X60.e.mais")

matriz$"X60.e.mais" <- as.numeric(matriz$"X60.e.mais")
matriz$matriculas_creche_municipal <- as.numeric(matriz$matriculas_creche_municipal)
matriz$matriculas_ESAI_estadual <- as.numeric(matriz$matriculas_ESAI_estadual)
matriz$matriculas_ESAF_municipal <- as.numeric(matriz$matriculas_ESAF_municipal)
matriz$matriculas_EM_municipal <- as.numeric(matriz$matriculas_EM_municipal)

pairs(matriz)

matriz2 = cbind(log(Total_de_crimes), log(Total_de_crimes1m), log(Total_de_crimes2m), log(População), log('60 e mais'), log(matriculas_creche_municipal), log(matriculas_ESAI_estadual), log(matriculas_ESAF_municipal), log(matriculas_EM_municipal))
colnames(matriz2) <- c('log(Totaldecrimes)','log(Totaldecrimes1m)', 'log(Totaldecrimes2m)', 'log(População)', 'log(sessenta_e_mais)', 'log(matriculas_creche_municipal)', 'log(matriculas_ESAI_estadual)', 'log(matriculas_ESAF_municipal)', 'log(matriculas_EM_municipal)')
pairs(matriz2)



########################################################## regressao sar 
agregado_tudo_3 <- agregado_tudo_novo_1[, c('crime_per_capita','Total_de_crimes1m', 'Total_de_crimes2m','População',
...
agregado_tudo_3$crime_per_capita <- as.numeric(agregado_tudo_3$crime_per_capita)
agregado_tudo_3$Total_de_crimes1m <- as.numeric(agregado_tudo_3$Total_de_crimes1m)
agregado_tudo_3$Total_de_crimes2m <- as.numeric(agregado_tudo_3$Total_de_crimes2m)
...
df_2 <- agregado_tudo_novo_2[c("name", "Total_de_crimes", "tempo", "População", "crime_per_capita")]


write.csv(df_1, "agregado_tudo_novo_5.csv", row.names = FALSE)
View(agregado_tudo_novo_5)




nmunicipio_2 <- municipio_1[rep(seq_len(nrow(municipio_1)), each = 12), ]  # Base R
...
Xis_3 <-  agregado_tudo_novo_2 %>% dplyr::select( "Total_de_crimes1m", "Total_de_crimes2m","População",
                                            "60 e mais","matriculas_creche_municipal","matriculas_ESAI_estadual"
                                            ,"matriculas_ESAF_municipal","matriculas_EM_municipal")
Xis_5 <-  agregado_tudo_novo_2 %>% dplyr::select("crime_per_capita")
...
Xis_3$Total_de_crimes1m <- as.numeric(Xis_3$Total_de_crimes1m)
Xis_3$Total_de_crimes2m <- as.numeric(Xis_3$Total_de_crimes2m)
Xis_3$População <- as.numeric(Xis_3$População)
Xis_3$"60 e mais" <- as.numeric(Xis_3$"60 e mais")
Xis_3$matriculas_creche_municipal <- as.numeric(Xis_3$matriculas_creche_municipal)
Xis_3$matriculas_ESAI_estadual <- as.numeric(Xis_3$matriculas_ESAI_estadual)
Xis_3$matriculas_ESAF_municipal <- as.numeric(Xis_3$matriculas_ESAF_municipal)
Xis_3$matriculas_EM_municipal <- as.numeric(Xis_3$matriculas_EM_municipal)


Xis_3$Total_de_crimes1m <- log(Xis_3$Total_de_crimes1m + 0.01)
Xis_3$Total_de_crimes2m <- log(Xis_3$Total_de_crimes2m + 0.01)
Xis_3$População <- log(Xis_3$População + 0.01)
Xis_3$"60 e mais" <- log(Xis_3$"60 e mais" + 0.01)
Xis_3$matriculas_creche_municipal <- log(Xis_3$matriculas_creche_municipal + 0.01)
Xis_3$matriculas_ESAI_estadual <- log(Xis_3$matriculas_ESAI_estadual + 0.01)
Xis_3$matriculas_ESAF_municipal <- log(Xis_3$matriculas_ESAF_municipal + 0.01)
Xis_3$matriculas_EM_municipal <- log(Xis_3$matriculas_EM_municipal + 0.01)


#transformar na para log(0.01) 


Xis_3$Total_de_crimes1m[is.na(Xis_3$Total_de_crimes1m)] <- 0.01
Xis_3$Total_de_crimes2m[is.na(Xis_3$Total_de_crimes2m)] <- 0.01
...
plot_predicted <- ggplot(test_data_sf) +
  geom_sf(aes(fill = test_data_sf$predicted, geometry = geometry)) +
...
plot_exp_y_chapeu <- ggplot(test_data_sf) +
  ...
  scale_fill_gradient(low="white", high="blue", name="predicted") +
  labs(title="Choropleth plot of SAR model about crime per capita \n on the metropolitan area of São Paulo")
