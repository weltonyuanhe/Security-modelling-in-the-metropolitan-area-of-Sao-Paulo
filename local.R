...
agregado_tudo_lasso <- agregado_tudo[ , -which(names(agregado_tudo) %in% c("Cod IBGE","name"
                                                                           ,"tempo","Unnamed: 0","Localidades","Área (Em km2)","Densidade Demográfica (Habitantes/km2)"
                                                                           ,"Grau de Urbanização (Em %)","Hab./Domicílio","Total H","Total M","00 a 14","15 a 29","30 a 59","60 e mais"
                                                                           ,"Cod_Ibge_x","Localidade_x","cod municipio","Localidade_y","Cod_Ibge_y","cod_ibge_x","Cod_Ibge","cod_ibge_y" 
                                                                           ))]
...
municipio <- subset(municipio, name != "Santa Bárbara d'Oeste")
municipio <- subset(municipio, name != "Santa Rita d'Oeste")
municipio <- subset(municipio, name != "São João do Pau d'Alho")
municipio <- subset(municipio, name != "São Luís do Paraitinga")
...
nmunicipio <- municipio[rep(seq_len(nrow(municipio)), each = 24), ]  # Base R
...
plot(queens, coordinates(municipio), pch = 19, cex = 0.6, add = TRUE, col = "red")
...
# VTI_texteis + VTI_vestuario + VTI_couro + VTI_celulose + VTI_impressao + VTI_petroleo + VTI_biocombustíveis + VTI_quimica + VTI_farmaceutica + VTI_min_n_met + VTI_metalurgia +VTI_prod_div +tx_abandono_fai_publica   + tx_abandono_fai_privada+ tx_aprovacao_faf_publica_x + tx_reprovacao_faf_publica + tx_abandono_faf_publica  + tx_reprovacao_faf_privada + tx_abandono_faf_privada +tx_abandono_medio_publica+ matriculas_creche_estadual
reg3 <- lagsarlm(formula =
                ...
                # + Total_de_crimes11m +
                   Total_de_crimes12m #+ População + area + Grau + habdom 
                # + trinta_a_cinquentanove + sessenta_e_mais + Valor_Agropecuaria + Valor_Serviços
                # + Valor_Adi + Valor_PIB_per_capita + VTI_bebidas + VTI_metal + VTI_outros_equi
            ...
            #     + tx_aprovacao_medio_publica + tx_reprovacao_medio_publica 
             #    + tx_aprovacao_medio_privada  + tx_reprovacao_medio_privada
              #   + tx_abandono_medio_privada  + matriculas_creche_municipal 
              ...
              #   + matriculas_EM_estadual + matriculas_EM_municipal 
              #   + matriculas_EM_particular, 
                 ,data = agregado_tudo, listw = a2listw,na.action = na.omit)
...
X <- as.matrix(data.frame(agregado_tudo_lasso[,! names(agregado_tudo_lasso) %in% cols.dont.want, drop = F]))
X[is.na(X)] <- 0
y[is.na(y)] <- 0
